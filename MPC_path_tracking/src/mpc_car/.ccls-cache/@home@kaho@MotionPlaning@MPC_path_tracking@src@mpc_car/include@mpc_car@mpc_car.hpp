...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    // Ad_ << 1, 0,  -v * std::sin(phi) * dt_, std::cos(phi) * dt_,
    //        0, 1,  v * std::cos(phi) * dt_, std::sin(phi) * dt_,
    //        0, 0,  1, std::tan(delta) * dt_ / ll_,
    //        0, 0,  0, 1;

    // Bd_ << 0, 0,
    //        0, 0,
    //        0, (v / ll_) / pow(std::cos(delta), 2) * dt_,
    //        dt_, 0;

    // gd_ << v * phi * std::sin(phi) * dt_,
    //        -v * phi * std::cos(phi) * dt_,
    //        -v / ll_ * delta / pow(std::cos(delta), 2) * dt_,
    //        0;
    // ...
    ...
    Ac.coeffRef(0, 2) = -v * std::sin(phi);
    Ac.coeffRef(0, 3) = std::cos(phi);
    Ac.coeffRef(1, 2) = v * std::cos(phi);
    Ac.coeffRef(1, 3) = std::sin(phi);
    Ac.coeffRef(2, 3) = std::tan(delta) / ll_;

    Bc.coeffRef(2, 1) = (v / ll_) / pow(std::cos(delta), 2);
    Bc.coeffRef(3, 0) = 1.0;

    gc.coeffRef(0) = v * phi * std::sin(phi);
    gc.coeffRef(1) = -v * phi * std::cos(phi);
    gc.coeffRef(2) = (-v / ll_) * delta / pow(std::cos(delta), 2);
  ...
  VectorX compensateDelay(const VectorX& x0) {
    ...
    // int delay_step = std::ceil(delay_ / dt_);
    ...
    //   }
    //   last_phi = phi; 
    //   x0_delay = Ad_ * x0_delay + Bd_ * historyInput_[i] + gd_;
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    Ad_.setZero();  // Ad for instance
    Bd_.setZero();
    gd_.setOnes();
    // ...
    // set size of sparse matrices
    P_.resize(m * N_, m * N_); // Q
    q_.resize(m * N_, 1); // R
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      Cu_.coeffRef(i * 4 + 0, i * m + 0) = 1;
      lu_.coeffRef(i * 4 + 0, 0) = -a_max_;
      uu_.coeffRef(i * 4 + 0, 0) = a_max_;
      // ...
      // -delta_max <= delta <= delta_max
      Cu_.coeffRef(i * 4 + 1, i * m + 1) = 1;
      lu_.coeffRef(i * 4 + 1, 0) = -delta_max_;
      uu_.coeffRef(i * 4 + 1, 0) = delta_max_;

      // -ddelta_max <= ddelta <= ddelta_max
      if(i < N_ - 1) {
        Cu_.coeffRef(i * 4 + 2, i * m + 1) = -1.0 / dt_;
        Cu_.coeffRef(i * 4 + 2, (i + 1) * m + 1) = 1.0 / dt_;
        lu_.coeffRef(i * 4 + 2, 0) = -ddelta_max_;
        uu_.coeffRef(i * 4 + 2, 0) = ddelta_max_;

        Cu_.coeffRef(i * 4 + 3, i * m) = -1.0 / dt_;
        Cu_.coeffRef(i * 4 + 3, (i + 1) * m) = 1.0 / dt_;
        lu_.coeffRef(i * 4 + 3, 0) = -2 * a_max_;
        uu_.coeffRef(i * 4 + 3, 0) = 2 * a_max_;
      } else {
        Cu_.coeffRef(i * 4 + 2, (i - 1) * m + 1) = 0;
        Cu_.coeffRef(i * 4 + 2, i * m + 1) = 0;

        Cu_.coeffRef(i * 4 + 3, i * m ) = 0;
        Cu_.coeffRef(i * 4 + 3, (i - 1) * m) = 0;
        
        lu_.coeffRef(i * 4 + 3, 0) = -a_max_;
        uu_.coeffRef(i * 4 + 3, 0) = a_max_;
        lu_.coeffRef(i * 4 + 2, 0) = -ddelta_max_;
        uu_.coeffRef(i * 4 + 2, 0) = ddelta_max_;
      }
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      std::cout << "match point x: " << xy(0) << std::endl;
      std::cout << "match point y: " << xy(1) << std::endl;
      std::cout << "match point phi: " << phi << std::endl;
      std::cout << "match point v: " << v << std::endl;
      
      s0 += desired_v_ * dt_;
      s0 = s0 < s_.arcL() ? s0 : s_.arcL();
    }
    
    std::cout << "_____________Ad_____________________" << std::endl;
    std::cout << Ad_ << std::endl;
    std::cout << "_____________Bd_____________________" << std::endl;
    std::cout << Bd_ << std::endl;
    std::cout << "________________BBMatrix______________________" << std::endl;
    std::cout << BB << std::endl;
    std::cout << "________________AAMatrix______________________" << std::endl;
    std::cout << AA << std::endl;
    std::cout << "________________ggMatrix______________________" << std::endl;
    ...
    q_ = BBT_sparse * Qx_.transpose() * (AA_sparse * x0_sparse + gg_sparse) - BBT_sparse * Qx_.transpose() * qx;
    // osqp
